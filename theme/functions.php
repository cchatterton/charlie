  ...
  static function section( $args ){
	...
	static function style(){
		...
		if ( false === ( $bb_style = get_transient( 'bb_style' ) ) ) {
			...
			$bb_style  = '/* last updated: '.date('c').' */'."\n";
			$bb_style .= file_get_contents( get_template_directory_uri().'/css/default.css' )."\n";
			$bb_style .= file_get_contents( get_template_directory_uri().'/css/style.css' );

			// Put the results in a transient. Expire after 12 hours.
			set_transient( 'bb_style', $bb_style, 12 * HOUR_IN_SECONDS );
